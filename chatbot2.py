...
def chatbot(query, questions, answers, vectorizer, tfidf_matrix):
    try:
        masked_q_for_logs = mask_user_input(query)
        log_masked_input(masked_q_for_logs)

        # Mask full payload before similarity processing
        masked_query = mask_sensitive_data(query)
        query_vec = vectorizer.transform([masked_query])
        similarities = cosine_similarity(query_vec, tfidf_matrix)

        print("Similarities: ", similarities)  # Debug: check matching quality
